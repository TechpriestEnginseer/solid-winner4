...
public class eis_modPlugin extends BaseModPlugin {
    ...
    public void onNewGame() {
        if (haveNex && SectorManager.getCorvusMode()) {
            ...
            naraka_b.autoUpdateHyperLocationBasedOnInSystemEntityAtRadius(system.getStar(), 0);
    ...
    public void onGameLoad(boolean newGame) {
        ...
        if (newGame) {
            if ("eis_valorous".equals(Global.getSector().getPlayerFleet().getFlagship().getHullSpec().getBaseHullId())) {
	...
	public void reportEconomyTick(int iterIndex) { //Calculate all the taxation
            if (Global.getSector().getMemoryWithoutUpdate().getFloat("$EIS_taxburden") <= 40000000) {
                ...
                if (Global.getSector().getPlayerFleet().getCargo().getCredits().get() >= 2000000f) {
                    ...
                    if ("hegemony".equals(Misc.getCommissionFactionId()) || "ironshell".equals(Misc.getCommissionFactionId()) || AllianceManager.getPlayerAlliance(true) == AllianceManager.getFactionAlliance("ironshell")) {
                        if (Global.getSector().getMemoryWithoutUpdate().getFloat("$EIS_PayrollTax") <= Global.getSector().getMemoryWithoutUpdate().getFloat("$EIS_WealthTax")+(SectorManager.getManager().isHardMode() ? 20000f : 40000f)) {
