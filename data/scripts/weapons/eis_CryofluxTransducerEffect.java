 ...
 */
public class eis_CryofluxTransducerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
																					EveryFrameWeaponEffectPlugin {

	public eis_CryofluxTransducerEffect() {
	}
	
	protected List<eis_CryofluxTransducerEffect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<eis_CryofluxTransducerEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (eis_CryofluxTransducerEffect curr : trails) {
				if (curr.proj != null) {
			...
			if (weight > 0.1f) {
				...
				Global.getSoundPlayer().playLoop("eis_fluxthrower_loop", ship, 1f, volume, com, ship.getVelocity());
			}
		}
		...
		for (int i = 0; i < numIter; i++) { 
			for (eis_CryofluxTransducerEffect trail : trails) {
	...
	public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
		/*if (projectile.getSource().getFluxLevel() > 0.75f) {engine.applyDamage(target, point, 50f*eis_lib.getDamageTypeMult(projectile.getSource(), target, WeaponType.ENERGY, false), DamageType.HIGH_EXPLOSIVE, 0, false, false, null);} else {engine.applyDamage(target, point, 50f*projectile.getSource().getFluxLevel()*eis_lib.getDamageTypeMult(projectile.getSource(), target, WeaponType.ENERGY, false), DamageType.HIGH_EXPLOSIVE, 0, false, false, null);engine.addFloatingText(point, "penises2", 12f, Color.yellow, target, 12f, 12f);}*/
                engine.addFloatingText(point, ""+projectile.getDamage().getModifier().modified, 10f, Color.yellow, target, 12f, 12f);
...
//								   rampUp, dur, inverted);
		...
		Misc.playSound(damageResult, point, vel,
				"eis_fluxthrower_hit_shield_light",
				"eis_fluxthrower_hit_shield_solid",
				"eis_fluxthrower_hit_shield_heavy",
				"eis_fluxthrower_hit_light",
				"eis_fluxthrower_hit_solid",
				"eis_fluxthrower_hit_heavy");
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		eis_CryofluxTransducerEffect trail = new eis_CryofluxTransducerEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<eis_CryofluxTransducerEffect>();
		}
		trails.add(0, trail);
	}
...
//			offset.y += vel.y * 1f;
	...
	public eis_CryofluxTransducerEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;
