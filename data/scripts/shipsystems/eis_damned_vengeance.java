...
public class eis_damned_vengeance extends BaseShipSystemScript {
    ...
    public StatusData getStatusData(int index, State state, float effectLevel) {
        if (index == 0 && state == State.OUT && reflectSuccess && activeTime < BUFF_DURATION)
            return new StatusData("VENGEANCE CORE SUPERCHARGED", false);
        if (index == 1 && state == State.OUT && reflectSuccess && activeTime < BUFF_DURATION)
            return new StatusData("SYSTEMS OPTIMIZED: " + Misc.getRoundedValueMaxOneAfterDecimal(BUFF_DURATION - activeTime) + "", false);
        if (index == 0 && state == State.OUT && !reflectSuccess && activeTime < DEBUFF_DURATION)
            return new StatusData("VENGEANCE CORE CRITICAL", true);
        if (index == 1 && state == State.OUT && !reflectSuccess && activeTime < DEBUFF_DURATION)
            return new StatusData("SYSTEMS WEAKENED: " + Misc.getRoundedValueMaxOneAfterDecimal(DEBUFF_DURATION - activeTime) + "", true);
        return null;
    }
