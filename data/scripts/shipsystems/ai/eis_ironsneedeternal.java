...
public class eis_ironsneedeternal implements ShipSystemAIScript {
    ...
    private final float EFFECT_RANGE = 330f; //300f is the effect range but we want to buff it so the AI can better predict things
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        if (tracker.intervalElapsed()) {
            ...
            List<MissileAPI> allMissiles = CombatUtils.getMissilesWithinRange(ship.getLocation(), ship.getCollisionRadius()+ship.getMutableStats().getSystemRangeBonus().computeEffective(EFFECT_RANGE) * MathUtils.getRandomNumberInRange(0.6f,1f));
            for (MissileAPI missile : allMissiles) {
                if (missile.getOwner() != ship.getOwner()) {
                    ...
                    switch (missile.getDamageType()) {
                        ...
                        if (missile.getDamageAmount() >= 1000f) {
                            missileThreatAmount += 3;
                        } else {missileThreatAmount += 1;}
                    }
                }
            if (desire >= MathUtils.getRandomNumberInRange(4f,5f) || missileThreatLevel >= MathUtils.getRandomNumberInRange(0.3f,1.2f) * ship.getHitpoints() * ((1-ship.getFluxTracker().getFluxLevel()))/2.5 || missileThreatAmount > 2) {
                ship.useSystem();
                desire = 0f;
            }
        }
