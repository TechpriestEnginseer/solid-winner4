...
public class eis_zandatsu extends BaseShipSystemScript {
    ...
    public static final float INSTANT_BOOST_FLAT = 675f; //from 500f
    public static final float INSTANT_BOOST_MULT = 3f; //from 5f
    ...
    private static String poopystinky = Global.getSettings().getString("eis_ironshell", "eis_jump_failed");
    private static String poopystinky2 = Global.getSettings().getString("eis_ironshell", "eis_zandatsu");
    private static String poopystinky3 = Global.getSettings().getString("eis_ironshell", "eis_zandatsu2");
    private static String poopystinky4 = Global.getSettings().getString("eis_ironshell", "eis_zandatsu3");
    ...
    private static final float ROF_BONUS = 1.33f;
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if (state == State.IDLE) {
            ...
            if (activeTime2 > 0f) {
                activeTime2 -= amount;
		stats.getAcceleration().modifyPercent(id+"2", 50f);
    ...
    public float getRegenOverride(ShipAPI ship) {
        if (ship != null) {
            return (0.2f/ship.getMutableStats().getSystemRegenBonus().getBonusMult());// why would someone... do a modifyFlat? wtf? -ship.getMutableStats().getSystemRegenBonus().flatBonus;
        }
        return -1;
    }
    ...
    public StatusData getStatusData(int index, State state, float effectLevel) {
        ...
        if (index == 1 && reflectSuccess && activeTime > 0f)
            return new StatusData(poopystinky3 + Misc.getRoundedValueMaxOneAfterDecimal(activeTime), false);
        if (index == 2 && activeTime2 > 0f)
            return new StatusData(poopystinky4 + Misc.getRoundedValueMaxOneAfterDecimal(activeTime2), false);
        return null;
    }
