...
public class eis_zandatsu extends BaseShipSystemScript {
    ...
    public float getRegenOverride(ShipAPI ship) {
        if (ship != null) {
            return (0.164f*(ship.getMutableStats().getSystemRegenBonus().getBonusMult() > 1 ? (ship.getMutableStats().getSystemRegenBonus().getBonusMult()+1f)/(2f*ship.getMutableStats().getSystemRegenBonus().getBonusMult()) : 1f));// why would someone... do a modifyFlat? wtf? -ship.getMutableStats().getSystemRegenBonus().flatBonus;
        }
        return -1f;
    }
