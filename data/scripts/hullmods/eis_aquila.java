...
public class eis_aquila extends BaseHullMod {
    ...
    static {
        ...
        speedcheck.put(HullSize.FRIGATE, 67f);
        speedcheck.put(HullSize.DESTROYER, 67f);
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship != null) {
            aquila.addPara(aquilaText1b, 0f, Misc.getPositiveHighlightColor(),
                    ...
                    "" + Math.round(Math.min((Float) speed.get(hullSize), ship.getMaxSpeed() * 0.75f)));
            if (ship.getVariant().getHullSpec().getShieldType() != ShieldType.PHASE) {
                aquila.addPara(aquilaText4b, 0f, Misc.getPositiveHighlightColor(), Math.round(DAMAGE_BONUS) + "%");
            }
        }
