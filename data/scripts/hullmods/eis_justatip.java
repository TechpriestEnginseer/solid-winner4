            ...
            if (isForModSpec) {
                ...
                for (WeaponAPI weapon : ship.getAllWeapons()) {
                    if (weapon.getType() == WeaponType.MISSILE && !weaponlist.containsKey(weapon.getDisplayName()) && weapon.getDamage().getBaseDamage() >= damagenumber) {
                        ...
                        if (isokaytoadd) {if (!diditonce){tooltip.addSectionHeading("", Alignment.MID, 3f);diditonce = true;}tooltip.addPara("• %s: %s "+each, 1f, new Color[]{Misc.getMissileMountColor(),Misc.getPositiveHighlightColor()}, weapon.getDisplayName(),"+"+Misc.getRoundedValue(water));}
                        }
                }
