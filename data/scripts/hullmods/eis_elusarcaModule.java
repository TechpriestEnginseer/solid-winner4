...
// Selk'ed from Techpriest
public class eis_elusarcaModule extends BaseHullMod {
    ...
    private static void advanceChild(ShipAPI child, ShipAPI parent) {
        ...
        if (parent.getVariant().hasHullMod("unstableinjector")) {
            child.getMutableStats().getBallisticWeaponRangeBonus().modifyMult("eis_module_ui", 0.85f);
            child.getMutableStats().getEnergyWeaponRangeBonus().modifyMult("eis_module_ui", 0.85f);
            child.getMutableStats().getFighterRefitTimeMult().modifyPercent("eis_module_ui", 25f);
        } else {
            child.getMutableStats().getBallisticWeaponRangeBonus().unmodify("eis_module_ui");
            child.getMutableStats().getEnergyWeaponRangeBonus().unmodify("eis_module_ui");
            child.getMutableStats().getFighterRefitTimeMult().unmodify("eis_module_ui");
        }
        ...
        if (parent.getFluxTracker().isVenting()) {
            child.getMutableStats().getVentRateMult().unmodify("eis_vent_module");
            child.giveCommand(ShipCommand.VENT_FLUX, null, 0);
        } else if (!child.getFluxTracker().isVenting()) {
            child.getMutableStats().getVentRateMult().modifyMult("eis_vent_module", 0f);
        }
    ...
    private static void advanceParent(ShipAPI parent, List<ShipAPI> children) {
        ...
        if (ec != null) {
            ...
            /* CODE WORD SNEED
            parent.getMutableStats().getAcceleration().modifyMult("eis_boss_elusarca", enginePerformance);
            parent.getMutableStats().getDeceleration().modifyMult("eis_boss_elusarca", enginePerformance);
            parent.getMutableStats().getTurnAcceleration().modifyMult("eis_boss_elusarca", enginePerformance);
            parent.getMutableStats().getMaxTurnRate().modifyMult("eis_boss_elusarca", enginePerformance);
            parent.getMutableStats().getMaxSpeed().modifyMult("eis_boss_elusarca", enginePerformance);
            parent.getMutableStats().getZeroFluxSpeedBoost().modifyMult("eis_boss_elusarca", enginePerformance);*/
        }
    }
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        switch (ship.getHullSpec().getBaseHullId()) {
            case "eis_boss_elusarca_ll":
            case "eis_boss_elusarca_lr":
            case "eis_boss_elusarca_tr":
            case "eis_boss_elusarca_tl":
                for (String tmp : BLOCKED_FRONT) {
                    if (ship.getVariant().getHullMods().contains(tmp)) {
                        MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "eis_boss_elusarca");
                    }
                }
                break;
            default:
