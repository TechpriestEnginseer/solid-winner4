...
public class eis_avaritia extends BaseHullMod {
	...
	speed.put(HullSize.FRIGATE, 100f); //50f
	speed.put(HullSize.DESTROYER, 80f); //30f
	speed.put(HullSize.CRUISER, 70f); //20f
	speed.put(HullSize.CAPITAL_SHIP, 60f); //10f
        speed.put(HullSize.DEFAULT, 10f);
        BLOCKED_HULLMODS.add(HullMods.SAFETYOVERRIDES);
    }
    public static final float PERCENT_PER_VENT = 50f;
    public static final float MIN_VENT_PERCENT = 50f;
    //public static final float VENT_RATE_BONUS = 15f;
    //public static final float CAPACITOR_MULT = 2f;
    public static final float DAMAGE_SBUFF_PERCENT = 50f;
    public static final float DAMAGE_BUFF_PERCENT = 30f;
    public static final float DAMAGE_BUFF_DURATION = 2f;

    public static final String DATA_KEY = "eis_avaritia_data_key";

    public static String Icon = Global.getSettings().getSpriteName("ui", "icon_tactical_venting");
    public static String Title = Global.getSettings().getString("eis_ironshell", "eis_avaritiaTitle");
    public static String Text1 = Global.getSettings().getString("eis_ironshell", "eis_avaritiaText1");
    public static String Text2 = Global.getSettings().getString("eis_ironshell", "eis_avaritiaText2");
    ...
    public static String Text5 = Global.getSettings().getString("eis_ironshell", "eis_avaritiaText5");
    public static String Text6 = Global.getSettings().getString("eis_ironshell", "eis_avaritiaText6");
    public static String ApplicableText = Global.getSettings().getString("eis_ironshell", "eis_avaritiaApplicableText");
    public static String StatusTitle = Global.getSettings().getString("eis_ironshell", "eis_avaritiaStatusTitle");
    public static String StatusText = Global.getSettings().getString("eis_ironshell", "eis_avaritiaStatusText");
    public static String StatusText2 = Global.getSettings().getString("eis_ironshell", "eis_avaritiaStatusText2");
    public static Color Amongus = Global.getSettings().getColor("textFriendColor");
    public static Color SUS = new Color(255, 0, 191);
    public static final float MAX_GLOW_PERCENT = 0.8f;
    public static final float FADE_IN_OUT_TIME = 0.2f;
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (fluxTracker.isVenting() && !data.startedVenting) {
            ...
            if (data.fluxLevelWhenStartedVenting <= MIN_VENT_PERCENT * 0.01f) {
                ...
                fluxTracker.setCurrFlux((MIN_VENT_PERCENT * 0.01f) * fluxTracker.getMaxFlux());
            ...
            if (ship.getHullSpec().isBuiltInMod("eis_avaritia") || ship.getVariant().getSMods().contains("eis_avaritia")) {
                ...
                stats.getAcceleration().modifyFlat("eis_avaritia", (Float) speed.get(hullSize) * 3f);
                stats.getDeceleration().modifyFlat("eis_avaritia", (Float) speed.get(hullSize) * 3f);
        ...
        if (engine.getPlayerShip() == ship && fluxTracker.getFluxLevel() > 0.5f) {
            engine.maintainStatusForPlayerShip(data.buffId + " 2", Icon, "Avaritia Ready",
                   "Vent to remove 50% flux", false);
        }
        ...
        if (data.startedVenting && engine.getPlayerShip() == ship) {
            ...
            MagicUI.drawHUDStatusBar(ship, fluxTracker.getFluxLevel(), Amongus, Amongus, data.fluxLevelToVentTo, String.valueOf(Math.round(ship.getMaxFlux() * (fluxTracker.getFluxLevel() - data.fluxLevelToVentTo)/(2*ship.getMutableStats().getFluxDissipation().getModifiedValue() * ship.getMutableStats().getVentRateMult().getModifiedValue()))) + " Seconds Remaining", ".", true);
        }
        ...
        if (data.startedVenting && (fluxTracker.getFluxLevel() <= Math.max(0.01f,data.fluxLevelToVentTo))) {
            ...
            data.fluxLevelToVentTo = 0f;
            data.buffDurationRemaining = DAMAGE_BUFF_DURATION;
            Global.getSoundPlayer().playSound("ai_core_pickup", 1f, 1f, ship.getLocation(), ship.getVelocity());
        }
        ...
        if (data.buffDurationRemaining > 0f) {
            ...
            if (ship.getHullSpec().isBuiltInMod("eis_avaritia") || ship.getVariant().getSMods().contains("eis_avaritia")) {
                ...
                if (ship == Global.getCombatEngine().getPlayerShip()) {
                    Global.getCombatEngine().maintainStatusForPlayerShip(data.buffId,"graphics/icons/hullsys/high_energy_focus.png", StatusTitle,"+" + Math.round(DAMAGE_SBUFF_PERCENT) + StatusText2, false);
                }
            } else {
                ...
                if (ship == Global.getCombatEngine().getPlayerShip()) {
                    Global.getCombatEngine().maintainStatusForPlayerShip(data.buffId,"graphics/icons/hullsys/high_energy_focus.png", StatusTitle,"+" + Math.round(DAMAGE_BUFF_PERCENT) + StatusText2, false);
                }
            }
            ...
            Global.getSoundPlayer().playLoop("system_high_energy_focus_loop", ship, 1f, 1f, ship.getLocation(), ship.getVelocity());
            ...
            // i'll kill you.
            if (data.buffDurationRemaining > DAMAGE_BUFF_DURATION - FADE_IN_OUT_TIME) {
                ship.setWeaponGlow(Math.min(MAX_GLOW_PERCENT,-MAX_GLOW_PERCENT/FADE_IN_OUT_TIME*data.buffDurationRemaining + DAMAGE_BUFF_DURATION*MAX_GLOW_PERCENT/FADE_IN_OUT_TIME), SUS, WEAPON_TYPES);
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        avaritia.addPara(Text5, 0f, Misc.getPositiveHighlightColor(), Math.round(DAMAGE_BUFF_PERCENT) + "%", Math.round(DAMAGE_BUFF_DURATION) + "");
        //JACKIEEEEEEEEEEEE
        if (isForModSpec) {
            avaritia.addPara(Text6, 0f, Misc.getGrayColor(), Misc.getHighlightColor(), "80", Math.round(DAMAGE_SBUFF_PERCENT) + "%");
	} else if (ship.getHullSpec().isBuiltInMod("eis_avaritia") || ship.getVariant().getSMods().contains("eis_avaritia")) {
            avaritia.addPara(Text6, 0f, Misc.getPositiveHighlightColor(), Misc.getHighlightColor(), String.valueOf(Math.round((float)speed.get(hullSize))), Math.round(DAMAGE_SBUFF_PERCENT) + "%");
        } else if (!isForModSpec) {
            avaritia.addPara(Text6, 0f, Misc.getGrayColor(), Misc.getHighlightColor(), String.valueOf(Math.round((float)speed.get(hullSize))), Math.round(DAMAGE_SBUFF_PERCENT) + "%");
	}
