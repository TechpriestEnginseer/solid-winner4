...
public class eis_avaritia extends BaseHullMod {
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (data.startedVenting && engine.getPlayerShip() == ship) {
            ...
            //engine.maintainStatusForPlayerShip(data.buffId + " 2", Icon, "Avaritia Overhaul",
                   //"Removing " + Math.round(ship.getMaxFlux() * (fluxTracker.getFluxLevel() - data.fluxLevelToVentTo)) + " flux until cancelled", true);
            if (ship.getHullSpec().isBuiltInMod("eis_avaritia") || ship.getVariant().getSMods().contains("eis_avaritia")) {
                engine.maintainStatusForPlayerShip("eis_avaritia", Icon, "Avaritia Overhaul", "improved maneuverability", true);
            }
            MagicUI.drawHUDStatusBar(ship, fluxTracker.getFluxLevel(), Amongus, Amongus, data.fluxLevelToVentTo, String.valueOf(Math.round(ship.getMaxFlux() * (fluxTracker.getFluxLevel() - data.fluxLevelToVentTo)/(ship.getMutableStats().getFluxDissipation().modified * ship.getMutableStats().getVentRateMult().getModifiedValue()))) + " Seconds Remaining", "Avaritia", false);
        }
        ...
        if (data.buffDurationRemaining > 0f) {
            ...
            if (ship == Global.getCombatEngine().getPlayerShip() && data.buffDurationRemaining > 0f) {
                Global.getCombatEngine().maintainStatusForPlayerShip(data.buffId,
                        "graphics/icons/hullsys/high_energy_focus.png", "Avaritia Overhaul",
                        "+" + Math.round(DAMAGE_BUFF_PERCENT) + "% ballistic and energy damage", false);
            }
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (isForModSpec) {
            ...
            avaritia.addPara(Text6, 0f, Misc.getPositiveHighlightColor(), Misc.getHighlightColor(), String.valueOf(Math.round((float)speed.get(hullSize))));
        } else if (!isForModSpec) {
            avaritia.addPara(Text6, 0f, Misc.getGrayColor(), Misc.getHighlightColor(), String.valueOf(Math.round((float)speed.get(hullSize))));
	}
