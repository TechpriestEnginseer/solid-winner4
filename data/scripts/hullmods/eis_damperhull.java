...
public class eis_damperhull extends BaseHullMod {
    ...
    static {
        ...
        duration.put(HullSize.CRUISER, 3f);
        duration.put(HullSize.CAPITAL_SHIP, 3.5f);
        armor.put(HullSize.FIGHTER, 0f);
        armor.put(HullSize.FRIGATE, 50f);
        armor.put(HullSize.DESTROYER, 90f);
        armor.put(HullSize.CRUISER, 120f);
        armor.put(HullSize.CAPITAL_SHIP, 150f);
        BLOCKED_HULLMODS.add(HullMods.HEAVYARMOR);
    }
    private final float lonelyaside = 50f;
    private final float cooldown = 15f;
    ...
    public boolean isApplicableToShip(ShipAPI ship) {
        return !((ship.getVariant().getHullSize() == HullSize.FRIGATE) || (ship.getVariant().hasHullMod(HullMods.CIVGRADE) && !ship.getVariant().hasHullMod(HullMods.MILITARIZED_SUBSYSTEMS)));
    }
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (data.activeTime > 0f) {
            ...
            if (data.activeTime > 2f) {Global.getSoundPlayer().playSound("system_damper_loop", 1f, 0.4f, ship.getLocation(), ship.getVelocity());}
        ...
        if (engine.getPlayerShip() == ship) {
            ...
            if (data.activeTime > 0f) {
                ...
                if (ship.getPhaseCloak() != null && ship.getPhaseCloak().getId().equals("damper")) {engine.maintainStatusForPlayerShip(data.buffId+"2", "graphics/icons/hullsys/damper_field.png",
                            "Vanagloria", (int) lonelyaside + "% less damage taken", false);engine.maintainStatusForPlayerShip(data.buffId+"2", "graphics/icons/hullsys/damper_field.png",
                            "Vanagloria", "Charge " + Math.round((data.activeTime/data.maxActiveTime)*100) + "%", false);} 
                else {MagicUI.drawInterfaceStatusBar(ship, data.activeTime/data.maxActiveTime,Misc.getNegativeHighlightColor(),Misc.getNegativeHighlightColor(),data.activeTime/data.maxActiveTime,"Charge",Math.round((data.activeTime/data.maxActiveTime)*100));engine.maintainStatusForPlayerShip(data.buffId+"2", "graphics/icons/hullsys/damper_field.png",
                            "Vanagloria", (int) lonelyaside + "% less damage taken", false);}
            } else {
                if (data.cooldown >= data.maxcooldown) {engine.maintainStatusForPlayerShip(data.buffId, "graphics/icons/hullsys/damper_field.png",
                            "Vanagloria", vanagloriaStatusTexta+holdfirekey+vanagloriaStatusTextb, false);}
                if (ship.getPhaseCloak() != null && ship.getPhaseCloak().getId().equals("damper")) {engine.maintainStatusForPlayerShip(data.buffId+"2", "graphics/icons/hullsys/damper_field.png",
                            "Vanagloria", "Charge " + Math.round((data.cooldown/data.maxcooldown)*100) + "%", true);} else {MagicUI.drawInterfaceStatusBar(ship, data.cooldown/data.maxcooldown,null,null,data.cooldown/data.maxcooldown,"Charge",Math.round((data.cooldown/data.maxcooldown)*100));}
            }
            if (ship.isHoldFire() && data.cooldown>=data.maxcooldown && ship.getCurrentCR() > 0f && !ship.getFluxTracker().isOverloadedOrVenting()) {
                ...
                Global.getSoundPlayer().playSound("system_damper_loop", 1.1f, 0.7f, ship.getLocation(), ship.getVelocity());
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        vanagloria.addPara(vanagloriaText1b, 0f);
        ...
        if (ship != null) {
            if (ship.getShield() != null) {
                vanagloria.addPara(damperText3b, 0f, Misc.getPositiveHighlightColor(),ship.getHullSpec().getHullNameWithDashClass(),"" + Misc.getRoundedValueMaxOneAfterDecimal(cooldown/(ship.getShield().getFluxPerPointOfDamage()*ship.getMutableStats().getShieldDamageTakenMult().getModifiedValue())));
                ...
                vanagloria.addPara(damperText3b, 0f, Misc.getPositiveHighlightColor(),ship.getHullSpec().getHullNameWithDashClass(),"" + Misc.getRoundedValueMaxOneAfterDecimal(cooldown/1.5f));
            } else if (ship.getHullSpec().isPhase() && (ship.getPhaseCloak() != null || ship.getHullSpec().getHints().contains(ShipTypeHints.PHASE))){
                vanagloria.addPara(damperText3b, 0f, Misc.getPositiveHighlightColor(),ship.getHullSpec().getHullNameWithDashClass(),"" + Misc.getRoundedValueMaxOneAfterDecimal(cooldown/((ship.getPhaseCloak().getFluxPerSecond()+ship.getPhaseCloak().getFluxPerUse())*3f/ship.getMaxFlux())));
            } else {
    ...
    private float eis_damperhull3(ShipAPI ship) {
        if (ship.getShield() != null) {
            return (cooldown/(ship.getShield().getFluxPerPointOfDamage()*ship.getMutableStats().getShieldDamageTakenMult().getModifiedValue()));
        } else if (ship.getShield() == null && !ship.getHullSpec().isPhase() && (ship.getPhaseCloak() == null || !ship.getHullSpec().getHints().contains(ShipTypeHints.PHASE))) {
            return cooldown/1.5f;
        } else if (ship.getHullSpec().isPhase() && (ship.getPhaseCloak() != null || ship.getHullSpec().getHints().contains(ShipTypeHints.PHASE))){
            return cooldown/((ship.getPhaseCloak().getFluxPerSecond()+ship.getPhaseCloak().getFluxPerUse())*3f/ship.getMaxFlux());
        } else {
            return cooldown/0.3f;
        }
    }
