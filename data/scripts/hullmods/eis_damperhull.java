...
public class eis_damperhull extends BaseHullMod {
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        for (String tmp : BLOCKED_HULLMODS) {
            ...
            /*Temporary measure until MagicLib gets updated?*/
            if (ship.getVariant().getSMods().contains(tmp)) {
                MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), "eis_damperhull", tmp);
            }
        }
    }
    ...
    public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
        if (stats.getVariant().getHullSpec().isBuiltInMod("eis_damperhull") || stats.getVariant().getSMods().contains("eis_damperhull")) {
            stats.getEffectiveArmorBonus().modifyFlat(id, ((float)armor.get(hullSize))+baste);
        } else {
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship != null) {
            if (ship.getHullSpec().isBuiltInMod("eis_damperhull") || ship.getVariant().getSMods().contains("eis_damperhull")) {
                vanagloria.addPara(vanagloriaText1, 0f, Misc.getPositiveHighlightColor(),
                    "+" + Math.round((Float) armor.get(hullSize)+baste));
            } else {
                vanagloria.addPara(vanagloriaText1, 0f, Misc.getPositiveHighlightColor(),
        ...
        if (ship != null) {
            if (ship.getShield() != null) {
                ...
                vanagloria.addPara(damperText3b, 0f, Misc.getPositiveHighlightColor(),ship.getHullSpec().getHullNameWithDashClass(),"" + Misc.getRoundedValueMaxOneAfterDecimal(cooldown/((ship.getPhaseCloak().getFluxPerSecond()+ship.getPhaseCloak().getFluxPerUse())*5f/ship.getMaxFlux())));
            } else {
    ...
    private float eis_damperhull3(ShipAPI ship) {
        if (ship.getShield() != null) {
            return (cooldown/(((ship.getShield().getFluxPerPointOfDamage()*ship.getMutableStats().getShieldDamageTakenMult().getModifiedValue()) > 1.33f) ? 1.33f : (ship.getShield().getFluxPerPointOfDamage()*ship.getMutableStats().getShieldDamageTakenMult().getModifiedValue())));
        } else if (ship.getShield() == null && !ship.getHullSpec().isPhase() && (ship.getPhaseCloak() == null || !ship.getHullSpec().getHints().contains(ShipTypeHints.PHASE))) {
            return cooldown/1.33f;
        } else if (ship.getHullSpec().isPhase() && (ship.getPhaseCloak() != null || ship.getHullSpec().getHints().contains(ShipTypeHints.PHASE))){
            return cooldown/((ship.getPhaseCloak().getFluxPerSecond()+ship.getPhaseCloak().getFluxPerUse())*5f/ship.getMaxFlux());
        } else {
            return cooldown/0.25f;
        }
    }
