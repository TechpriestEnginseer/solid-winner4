...
public class eis_audaciousrelay extends BaseHullMod {
    ...
    private static final float SHIELD_EFF_PERCENT = 15f;
                ...
                if (ship.getShipAI() != null) {
                    for (ShipAPI ship2ai : shipinrange) {
                        if (ship2ai.getOwner() == ship.getOwner() && (ship2ai.getVariant().hasHullMod("eis_vengeance") || ship2ai.getVariant().hasHullMod("eis_perfect_vengeance"))) {
                            if (!ship.isRetreating() && engine.getFleetManager(ship.getOwner()).getTaskManager(ship.isAlly()).getAssignmentFor(ship) == null && !data.havingEggs) {engine.getFleetManager(ship.getOwner()).getTaskManager(ship.isAlly()).giveAssignment(engine.getFleetManager(ship.getOwner()).getDeployedFleetMember(ship), engine.getFleetManager(ship.getOwner()).getTaskManager(ship.isAlly()).createAssignment(CombatAssignmentType.DEFEND, engine.getFleetManager(ship.getOwner()).getDeployedFleetMember(ship2ai), false), false);data.havingEggs=true;}
                            break;
                        }
                    }
                }
