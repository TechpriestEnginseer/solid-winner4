...
public class eis_avaritia extends BaseHullMod {
	...
	speed.put(HullSize.FRIGATE, 75f); //50f
	speed.put(HullSize.DESTROYER, 65f); //30f
	speed.put(HullSize.CRUISER, 60f); //20f
	speed.put(HullSize.CAPITAL_SHIP, 55f); //10f
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (data.timer.intervalElapsed()) {
            ...
            if (decisionLevel >= 1.5f || (ship.getFluxTracker().getFluxLevel() >= 0.65f && dangerFactor == 0)) {
                //engine.addFloatingText(ship.getLocation(), "Flux NOW "+decisionLevel, 20f, SUS, ship, 1f, 1f);
                ship.giveCommand(ShipCommand.VENT_FLUX, null, 0);
                ship.getAIFlags().setFlag(ShipwideAIFlags.AIFlags.DO_NOT_BACK_OFF, 1.5f);
                ship.getAIFlags().setFlag(ShipwideAIFlags.AIFlags.DO_NOT_BACK_OFF_EVEN_WHILE_VENTING, 1.5f);
            }
        }
        // < end Stuff from Nia>
    }
    ...
    public boolean isApplicableToShip(ShipAPI ship) {
        if (ship.isFrigate() || ship.isDestroyer() || ship.isCruiser()) {
            return true;
        }
        return false;
    }
