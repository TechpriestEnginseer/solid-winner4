				...
				for (FactionAPI faction : group.factions)
				{
					...
					opts.addOption(Nex_FactionDirectoryHelper.getFactionDisplayName(faction), 
							MUSIC_PRINT_FACTION_OPTION_PREFIX + faction.getId(), faction.getBaseUIColor(), null);
                                        if (faction.getMusicMap().isEmpty()) {opts.setEnabled(MUSIC_PRINT_FACTION_OPTION_PREFIX + faction.getId(), false);opts.setTooltip(MUSIC_PRINT_FACTION_OPTION_PREFIX + faction.getId(), "They don't have music for you to vibe to.");}
				}
			...
			case "printMusic":
				String factionId = option.substring(PREFIX_LENGTH);
                                ...
                                if (Global.getSector().getFaction(factionId).getMusicMap().get("market_friendly") != null) {
                                    dialog.getOptionPanel().addOption("Friendly Market:" + Global.getSector().getFaction(factionId).getMusicMap().get("market_friendly"), "nexual_market_friendly");
                                }
                                if (Global.getSector().getFaction(factionId).getMusicMap().get("market_neutral") != null) {
                                    dialog.getOptionPanel().addOption("Neutral Market:" + Global.getSector().getFaction(factionId).getMusicMap().get("market_neutral"), "nexual_market_neutral");
                                }
                                if (Global.getSector().getFaction(factionId).getMusicMap().get("market_hostile") != null) {
                                    dialog.getOptionPanel().addOption("Hostile Market:" + Global.getSector().getFaction(factionId).getMusicMap().get("market_hostile"), "nexual_market_hostile");
                                }
                                if (Global.getSector().getFaction(factionId).getMusicMap().get("encounter_friendly") != null) {
                                    dialog.getOptionPanel().addOption("Friendly Encounter:" + Global.getSector().getFaction(factionId).getMusicMap().get("encounter_friendly"), "nexual_encounter_friendly");
                                }
                                if (Global.getSector().getFaction(factionId).getMusicMap().get("encounter_neutral") != null) {
                                    dialog.getOptionPanel().addOption("Neutral Encounter:" + Global.getSector().getFaction(factionId).getMusicMap().get("encounter_neutral"), "nexual_encounter_neutral");
                                }
                                if (Global.getSector().getFaction(factionId).getMusicMap().get("encounter_hostile") != null) {
                                    dialog.getOptionPanel().addOption("Hostile Encounter:" + Global.getSector().getFaction(factionId).getMusicMap().get("encounter_hostile"), "nexual_encounter_hostile");
                                }
