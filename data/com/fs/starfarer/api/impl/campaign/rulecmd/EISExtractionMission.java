...
public class EISExtractionMission extends HubMissionWithBarEvent {
	...
	protected boolean create(MarketAPI createdAt, boolean barEvent) {
		...
		makeImportant(getPerson(), "$eis_extr_returnHere", Stage.RETURN);
...
//		setCreditReward(10000 + sizeModifier, 30000 + sizeModifier);
	...
	public void addDescriptionForNonEndStage(TooltipMakerAPI info, float width, float height) {
		...
		if (currentStage == Stage.EXTRACT) {
			info.addPara("Extract operative located " +
...
//						"required to determine the forces necessary.", opad);
	...
	public boolean addNextStepText(TooltipMakerAPI info, Color tc, float pad) {
		Color h = Misc.getHighlightColor();
		if (currentStage == Stage.EXTRACT) {
				info.addPara("Extract operative from " +
							 market.getName() + 
