	...
	protected boolean create(MarketAPI createdAt, boolean barEvent) {
		...
		//assignShipName(member, Factions.INDEPENDENT);
                if (person.getTags().contains("eis_celeste")) {
                    ...
                    if (Math.random() >= 0.25f) {assignShipName(member, "ironshell");if (!member.getVariant().hasHullMod(HullMods.SAFETYOVERRIDES)) {member.getVariant().addPermaMod("eis_aquila", true);}} else {assignShipName(member, Factions.HEGEMONY);BASE_PRICE_MULT = 0.55f;}
                }
