	...
	protected boolean create(MarketAPI createdAt, boolean barEvent) {
		...
		//assignShipName(member, Factions.INDEPENDENT);
                if (person.getTags().contains("eis_celeste")) {
                    assignShipName(member, "ironshell");if (!member.getVariant().hasHullMod(HullMods.SAFETYOVERRIDES) && !member.getVariant().hasHullMod("eis_aquila")) {holybased=true;member.getVariant().addPermaMod("eis_aquila", true);member.getVariant().setSource(VariantSource.REFIT);variant.addTag(Tags.VARIANT_ALWAYS_RETAIN_SMODS_ON_SALVAGE);}
                } else {
                    if (MathUtils.getRandomNumberInRange(0f, 1f) >= 0.25f) {assignShipName(member, "ironshell");if (!member.getVariant().hasHullMod(HullMods.SAFETYOVERRIDES)) {holybased=true;member.getVariant().addPermaMod("eis_aquila", true);member.getVariant().addPermaMod("eis_aquila", true);member.getVariant().setSource(VariantSource.REFIT);variant.addTag(Tags.VARIANT_ALWAYS_RETAIN_SMODS_ON_SALVAGE);}} else {assignShipName(member, Factions.HEGEMONY);BASE_PRICE_MULT = 0.4f;}
                }
