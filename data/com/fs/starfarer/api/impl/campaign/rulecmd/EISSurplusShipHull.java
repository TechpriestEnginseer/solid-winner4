...
public class EISSurplusShipHull extends HubMissionWithBarEvent {

	public static float BASE_PRICE_MULT = 1.05f;
	...
	protected boolean create(MarketAPI createdAt, boolean barEvent) {
		...
		//assignShipName(member, Factions.INDEPENDENT);
                if (person.getTags().contains("eis_celeste")) {
                    assignShipName(member, "ironshell");if (!member.getVariant().hasHullMod(HullMods.SAFETYOVERRIDES) && !member.getVariant().hasHullMod("eis_aquila")) {member.getVariant().addPermaMod("eis_aquila", true);}
                } else {
                    if (Math.random() >= 0.25f) {assignShipName(member, "ironshell");if (!member.getVariant().hasHullMod(HullMods.SAFETYOVERRIDES)) {member.getVariant().addPermaMod("eis_aquila", true);}} else {assignShipName(member, Factions.HEGEMONY);BASE_PRICE_MULT = 0.95f;}
                }
