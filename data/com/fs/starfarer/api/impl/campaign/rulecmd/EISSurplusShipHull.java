	...
	protected boolean create(MarketAPI createdAt, boolean barEvent) {
		...
		//assignShipName(member, Factions.INDEPENDENT);
                if (person.getTags().contains("eis_celeste")) {
                    ...
                    if (MathUtils.getRandomNumberInRange(0f, 1f) >= 0.25f) {assignShipName(member, "ironshell");if (!member.getVariant().hasHullMod(HullMods.SAFETYOVERRIDES)) {holybased=true;member.getVariant().addPermaMod("eis_aquila", true);}} else {assignShipName(member, Factions.HEGEMONY);BASE_PRICE_MULT = 0.4f;}
                }
