	...
	public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Token> params, Map<String, MemoryAPI> memoryMap) {
            if (Global.getSettings().getModManager().isModEnabled("nexerelin")) {
                if (Global.getSector().getEntityById(params.get(0).getString(memoryMap)) != null && !Global.getSector().getEntityById(params.get(0).getString(memoryMap)).getMarket().getFactionId().equals("ironshell")) {
                    ...
                    DiplomacyManager.createDiplomacyEvent(Global.getSector().getFaction("ironshell"), Global.getSector().getFaction("shadow_industry"), "declare_war", null);
                    DiplomacyManager.createDiplomacyEvent(Global.getSector().getFaction("hegemony"), Global.getSector().getFaction("shadow_industry"), "declare_war", null);
