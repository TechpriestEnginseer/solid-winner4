...
public class LivingInPods extends BaseIntelPlugin {
	...
	public void createSmallDescription(TooltipMakerAPI info, float width, float height) {
		...
		if (entity.getFaction().equals(Global.getSector().getImportantPeople().getPerson(person).getFaction()) || entity.getFaction().getId().equals("player")) { Global.getSector().getImportantPeople().getPerson(person).getMemoryWithoutUpdate().unset(finishedsociety);info.addPara(finishedtext2, opad);
                } else {info.addPara(text, opad);}
	}
