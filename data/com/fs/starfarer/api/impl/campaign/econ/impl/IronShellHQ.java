...
public class IronShellHQ extends BaseIndustry implements RouteFleetSpawner, FleetEventListener {
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		case FAST:
			...
			break;
		case COMBAT:
			combat = Math.round(15f + (float) random.nextFloat() * 5f) * 5f; //from 6f, 5f
			//tanker = Math.round((float) random.nextFloat()); //* 5f;
			break;
		case HEAVY:
			combat = Math.round(20f + (float) random.nextFloat() * 5f) * 5f; //from 10f
			//tanker = Math.round((float) random.nextFloat()) * 2f; //10f
			//freighter = Math.round((float) random.nextFloat()) * 2f; //10f
			break;
		}
		...
		params.modeOverride = Misc.getShipPickMode(market);
                ...
                if (type == PatrolType.HEAVY) {params.flagshipVariantId = "eis_legion_xiv_Elite";}
                if (type == PatrolType.COMBAT) {params.flagshipVariantId = "eis_endeavor_elite";} //params.flagshipVariantId = "eis_tyrant_elite";
...
//		fleet.addScript(ai);
		...
		fleet.getCommander().setRankId(rankId);
                if (type == PatrolType.HEAVY) {
                    ...
                    //fleet.getFlagship().setVariant(Global.getSettings().getVariant("eis_legion_xiv_Elite"), true, true);
                ...
                if (type == PatrolType.COMBAT) {
                    ...
                    //fleet.getFlagship().setVariant(Global.getSettings().getVariant("eis_tyrant_elite"), false, true);
	...
	public void addImproveDesc(TooltipMakerAPI info, ImprovementDescriptionMode mode) {
		...
		if (mode == ImprovementDescriptionMode.INDUSTRY_TOOLTIP) {
			info.addPara("Pather Intrigue reduced by %s.", 0f, highlight, "4");
		} else {
			info.addPara("Reduces the Pather Intrigue by %s.", 0f, highlight, "4");
		}
